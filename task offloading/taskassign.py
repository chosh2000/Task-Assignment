...
def matrix2graph():
    ...
    #check if the graph is properlly made
    '''
    print 'Graph data:'
    for v in G:
        if bool(v.get_connections()) == 0:
            print ('%s, #########processor doesnt have a link################'% v.get_id())
        for w in v.get_connections():
            vid = v.get_id()
            wid = w.get_id()
            print '( %s, %s , %s, %3d)'  % (v.processorType, vid, wid, v.get_weight(w))
    '''
    return G
...
def create_task_graph():
    ...
    # Create task link, in KB
    G_task.add_edge(0, 1, 60)
    G_task.add_edge(0, 2, 40)
    G_task.add_edge(1, 3, 20)
    G_task.add_edge(1, 4, 30)
    G_task.add_edge(2, 5, 10)
    G_task.add_edge(2, 6, 20)
...
def getExecTime(task,processor,resol):
    ...
    if task.id == 0:
        if processor.processorType == 'desk':
            ...
            b = 0
            c = 0.0001
        elif processor.processorType == 'rpi':
            a = 0
            b = 0
            c = 0.005
        elif processor.processorType == 'jetson':
            a = 0
            b = 0
            c = 0.001
    elif task.id == 1:
...
def calc_f(node):
    ...
    for t in node.K:                         # for tasks assigned in the partial assignemnt
        if node.K[t] == q:                   # if assigned to current processor q
            print("task %d assigned to current processor %d" %(t.id, node.K[t].id))
...
def astar(taskGraph, processorGraph):
    ...
    while len(open_set)>0:
        ...
        for ct in expandingnode.current_task.adjacent:
            print("**************************connected task ********************: %d"%ct.id)
        for connectedtask in expandingnode.current_task.adjacent:
            if connectedtask.id in expandingnode.UT:
                nexttask = expandingnode.UT[connectedtask.id]          #next task = unassigned task connected to task in expanding node 
                print("#######################################################nexttask:")
                print(nexttask)
                ...
                search_Vertex = astarGraph.vert_dict[searchnode]
                search_Vertex.current_processor = processorGraph.vert_dict[expandingnode.current_processor.id]
                ...
                if len(expandingnode.K) == (len(taskGraph.vert_dict)-1):
                    final_set[searchnode] = search_Vertex



                '''assign it to adjacent processors'''
                for adjacentProcessor in expandingnode.current_processor.adjacent:       #get_connections gives the processor vertex
                    searchnode += 1
                    print("=============================")
                    ...
                    astarGraph.add_vertex(searchnode)
                    search_Vertex = astarGraph.vert_dict[searchnode]
                    search_Vertex.current_processor = processorGraph.vert_dict[adjacentProcessor.id]
                    search_Vertex.current_task = nexttask
                    ...
                    for p in expandingnode.K:
                        print("EK:<%d,%d>"%(p.id, expandingnode.K[p].id))

                    '''
                    Problem:
                    2) communication cost is not calculated
                    3) Tree doesn't expand until the end.
                    '''
    ...
    for final_task in final_set[final_solution].K:
        ...
        print("final task id = %d, final processor id = %d\n" % (final_task.id, final_set[final_solution].K[final_task].id))
        '''Update the turnaround time for this assignment '''
        final_processor.TA += calc_e(final_task,final_processor)
        # final_processor.TA += calc_c()
...
if __name__ == '__main__':
    ...
    # num_processor_node = 20
    # processor_matrix = generate_processor_link_matrix(num_processor_node, 0.4)  # 40% probability
