...
def create_task_graph(pd_resol, fd_resol):
    ...
    G_task.add_edge(0, 1, (0.0001*math.pow(pd_resol,2) + 0.0476*pd_resol - 3.9924)) # copy1 
    G_task.add_edge(0, 2, (0.0001*math.pow(fd_resol,2) + 0.0476*fd_resol - 3.9924)) # copy2
    G_task.add_edge(1, 3, (0.0001*math.pow(pd_resol,2) + 0.0476*pd_resol - 3.9924)) # copy1 -> pd
    G_task.add_edge(2, 4, (0.0001*math.pow(fd_resol,2) + 0.0476*fd_resol - 3.9924)) # copy2 -> fd
    G_task.add_edge(4, 5, (0.000002*math.pow(fd_resol,2) + 0.00008*fd_resol + 0.658)) # fd->fr
...
if __name__ == '__main__':
    ...
    # num_processor_node = 50
    # processor_matrix = generate_processor_link_matrix(num_processor_node, 0.20)  # 40% probability
    ...
    G_proc = matrix2graph()  # for astar task assignment
    G_proc2 = matrix2graph() # for without task assignment


    ''' step2) set modular resolution    ex)R = {0:600, 1:540, ...}'''
    R = {}
    R[0] = 640
    R[1] = 240 # change this: People Detect Resolution
    R[2] = 300 # change this: Face Detect Resolution
    R[3] = R[1]
    R[4] = R[2]
    R[5] = R[2]
    # R[6] = R[2]

    ''' step2) Create task graphs with different resolution  '''
    for counter in range(0,120): # counter == number of applications running
        G_task = create_task_graph(R[1], R[2]) #(pd_resolution, fd_resolution) 
